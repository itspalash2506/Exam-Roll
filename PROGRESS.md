...
- ~~Local storage only~~ — **Resolved 2026-09-20**: uploaded files and Excel outputs now live in Cloudflare R2 (object storage), not on the server's disk, whenever R2 is configured (falls back to local `uploads/` when it isn't — e.g. local dev). This is what fixed the growing memory/disk pressure the previous local-only storage caused on the deployed server. See `DECISIONS.md`.
